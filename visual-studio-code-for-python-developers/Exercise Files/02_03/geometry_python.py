...
def main():
    ...
    while (running):
        ...
        if choice == "X":
            ...
            length = float(response)
            ans = geometry_calculator.area_of_rect(w, length)
            print(
                f"Area of rectangle with width {w} and height {length} is {ans}")
        elif choice == "3":
            response = input("Width? ")
            w = float(response)
            response = input("Length? ")
            ...
            d = float(response)
            ans = geometry_calculator.volume_of_box(w, length, d)
            print(
                f"Volume of rectangle with width {w},height {length}, depth {d} is {ans}")
        elif choice == "4":
            response = input("Radius? ")
            r = float(response)
